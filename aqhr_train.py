...
def get_args_parser():
    ...
    parser = argparse.ArgumentParser('ViT-AE training for air quality high resolution dataset', add_help=False)
...
def train(args):
    ...
    # model dir
    output_dir = os.path.join(model_root, data_name, model_dir_name, input_type)
    os.makedirs(output_dir, exist_ok=True)

    # log dir
    logs_dir = os.path.join(log_root, data_name, model_dir_name, input_type)
    ...
    # dataset
    dataset_kwargs = dict(obs_mask=obs_mask, patch_size=patch_size,
                          input_type=input_type, mean_std=mean_std)
    dataset_train = AQHighResDataset(train_list, **dataset_kwargs)
    dataset_val = AQHighResDataset(val_list, **dataset_kwargs)
    input_size = dataset_train.input_size
    ...
    for epoch in range(start_epoch, epochs):
        ...
        train_stats = train_one_epoch(
            ...
            loss_scaler, args, opt_func, eval_func, mean_std
        )

        # epoch for validation
        val_stats = val_one_epoch(
            model, loader_val, device, epoch,
            args, opt_func, eval_func, mean_std
        )

        # save checkpoint regularly
        if epoch % ckpt_freq == 0 or epoch + 1 == epochs:
            misc.save_model(
                args=args, output_dir=output_dir, model=model,
