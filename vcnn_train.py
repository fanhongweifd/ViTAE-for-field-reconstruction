...
def get_args_parser():
    ...
    parser = argparse.ArgumentParser('ViT-AE training for Voronoi-CNN dataset', add_help=False)
...
def train(args):
    ...
    train_list = load_pickle(os.path.join(data_path, 'train.pkl'))
    val_list = load_pickle(os.path.join(data_path, 'val.pkl'))
    mean_std = load_pickle(os.path.join(data_path, 'mean_std.pkl')) if normalize else None

    # dataset
    dataset_train = VCNNDataset(train_list, input_type, mean_std, augment)
    dataset_val = VCNNDataset(val_list, input_type, mean_std)
